...
bool rangesOverlap(std::vector<int> r1, std::vector<int> r2)
{
	if (r1[0] <= r2[0])
		if (r1[1] >= r2[0])
			return true;
...
int main(int argc, char *argv[])
{
	...
	while (fgets(line, sizeof(line), file))
	{
		...
		if (rangesOverlap(section_one, section_two) || rangesOverlap(section_two, section_one))
			sum++;
	}

	fclose(file);
	std::cout << "Assignment pairs that overlap: " << sum << std::endl;
